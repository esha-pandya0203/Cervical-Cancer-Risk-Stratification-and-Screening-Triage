...
call_risk_api <- function(patient_row, low_thr = 0.15, high_thr = 0.35) {
  ...
  if (!api_is_configured()) {
    local_score$api_status <- "API URL not configured; using local model."
    return(local_score)
  }

  result <- tryCatch({
    api_key <- Sys.getenv("CERVICAL_API_KEY", unset = "")
    if (nzchar(api_key)) {
    ...
    resp <- httr::POST(
      ...
      headers,
      body = jsonlite::toJSON(build_api_payload(patient_row, low_thr, high_thr), auto_unbox = TRUE),
      encode = "raw",
    ...
    if (status >= 200 && status < 300) {
      ...
      normalize_api_score(api_body, local_score)
    } else {
      local_score$api_status <- paste0("API returned status ", status, "; using local model.")
      local_score
    }
  }, error = function(e) {
...
# ---------------------------
# Login credentials for prototype authentication
# ---------------------------
# These are demo/course-project credentials. For a real clinical application,
# passwords should be hashed and stored outside the app source code.
login_credentials <- data.frame(
  user = c("smitchell@clinic.org", "mgonzalez@clinic.org"),
  password = c("pass1", "pass3"),
  role = c("clinician", "admin"),
  stringsAsFactors = FALSE
)

check_login_credentials <- function(user, password) {
  matched <- login_credentials[
    login_credentials$user == user & login_credentials$password == password,
    ,
    drop = FALSE
  ]
  ...
  if (nrow(matched) == 1) {
    ...
    data.frame(result = FALSE, stringsAsFactors = FALSE)
  }
}
...
server <- function(input, output, session) {
  ...
  res_auth <- secure_server(check_credentials = check_login_credentials)
  current_user_role <- reactive({ res_auth$role %||% "unknown" })
  current_user_email <- reactive({ res_auth$user %||% "unknown" })
  ...
  observeEvent(input$save_new_patient, {
    tryCatch({
      name_input <- trimws(input$new_name %||% "")
      if (!nzchar(name_input)) {
        showNotification("Please enter a patient name before saving.", type = "error", duration = 5)
        return(NULL)
      }

      parts <- unlist(strsplit(name_input, "\\s+"))
      parts <- parts[nzchar(parts)]
      first_name <- if (length(parts) >= 1) parts[1] else "New"
      ...
      # Reset one-row template safely, then repopulate all fields used by the app.
      new_row[1, ] <- NA
      ...
      new_row$dob <- as.character(input$new_dob %||% Sys.Date())
      new_row$address <- input$new_address %||% ""
      new_row$city <- input$new_city %||% ""
      new_row$state <- "PA"
      rv$patients$zip <- as.character(rv$patients$zip)
      new_row$zip <- as.character(input$new_zip %||% "")
      new_row$phone <- input$new_phone %||% ""
      new_row$email <- paste0(tolower(gsub("[^A-Za-z0-9]", "", first_name)), ".", 
                              tolower(gsub("[^A-Za-z0-9]", "", last_name)), 
                              new_id, "@demohealth.org")

      new_row$age <- coalesce_num(input$new_age, 35)
      new_row$number_of_sexual_partners <- coalesce_num(input$new_partners, 1)
      new_row$first_sexual_intercourse_age <- coalesce_num(input$new_firstsex, 18)
      new_row$num_of_pregnancies <- coalesce_num(input$new_preg, 0)
      new_row$smokes <- coalesce_num(input$new_smokes, 0)
      new_row$smokes_years <- coalesce_num(input$new_smokes_years, 0)
      new_row$smokes_packs_per_year <- coalesce_num(input$new_smokes_packs, 0)
      new_row$hormonal_contraceptives <- coalesce_num(input$new_hc, 0)
      new_row$hormonal_contraceptives_years <- coalesce_num(input$new_hc_years, 0)
      new_row$iud <- coalesce_num(input$new_iud, 0)
      new_row$iud_years <- coalesce_num(input$new_iud_years, 0)
      new_row$stds <- coalesce_num(input$new_stds, 0)
      new_row$stds_number <- coalesce_num(input$new_stds_number, 0)
      new_row$dx_hpv <- coalesce_num(input$new_dx_hpv, 0)
      new_row$dx_cancer <- coalesce_num(input$new_dx_cancer, 0)
      new_row$dx_cin <- 0
      new_row$dx <- ifelse(new_row$dx_hpv + new_row$dx_cancer > 0, 1, 0)
      ...
      rv$patients <- dplyr::bind_rows(rv$patients, new_row)
      rv$selected_patient_id <- new_id
      rv$show_add_patient <- FALSE
      showNotification(paste("Added patient", new_row$patient_name), type = "message", duration = 4)
      updateNavbarPage(session, "main_navbar", selected = "Patient Risk Assessment")
    }, error = function(e) {
      showNotification(paste("Could not add patient:", conditionMessage(e)), type = "error", duration = 10)
      message("Add patient error: ", conditionMessage(e))
    })
  })
